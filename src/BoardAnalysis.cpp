...
Bitboard BoardAnalysis::getCheckMask (const Board& context, PieceColor const color) {
    ...
    // bishop and queen
    const Bitboard& supposedBishops = attacks.getBishopAttacks().getRaysToAllDirections(context, kingPosition, false) & ~context.getBlockers(color, false);
...
//                return checkMask;
    ...
    // rook and queen
    const Bitboard& supposedRooks = attacks.getRookAttacks().getRaysToAllDirections(context, kingPosition, false) & ~context.getBlockers(color, false);
